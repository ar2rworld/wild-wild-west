//same weapon
/*two states : scanning and shooting
 scanning : small movements on the high ground platform and searching on the big distance player
 shooting : one pose of enemy and floating weapon the LookAt() tool targets weapon which is independent from player
 difficulty : randomness of successful shoot; or shooting on the circle around the player's head, radius is smaller then difficulty is high.
 */
...
public class sniperEnemySCR : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if ((getDistanceToPlayer() < 20 && isPlayerVisible() || foundTarget) && !dead)
        {
            a.Play("sniperShooting");
            animStatus = 1;
            foundTarget = true;
            idle = false;
        ...
        {
            activeWeapon.SetActive(false);
            a.Play("sniperShootingIdle");
            animStatus = 0;
            float tempN0 = Time.time * 10;
            //Debug.Log(tempN0);
            //transform.rotation = new Quaternion(0, tempN0, 0,2f);
    ...
    bool isPlayerVisible()
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(origin, ray, out hit, activeDistance))
        {
            if (hit.collider.gameObject.tag == "Player" && Vector3.Angle(transform.forward, player.transform.position - transform.position) < 40 )
